...
消费价格指数（CPI）
原理推导：$\mathbf {(1+r_{\small{\textbf{名义}}})=(1+r_{\small{\textbf{实际}}})\times(1+r_{\small{\mathbf{CPI}}})}$  
公式 1：$\mathbf {r_{\small{\textbf{名义}}}=r_{\small{\textbf{实际}}}+r_{\small{CPI}}+r_{\small{CPI}}\times r_{\small{\textbf{实际}}}}$  
速算技巧：代入排除法  
...
3.具体值  
	① 给现期和基期  
		$\mathbf {r=\dfrac{\text{增长量}}{\text{基期量}}=\dfrac{\textbf{现期量-基期量}}{\textbf{基期量}}=\dfrac{\textbf{现期量}}{\textbf{基期量}}-1}$  
	② 给现期、增长量  
...
**增长量**  
判定：增长+具体单位（人/元/吨）  
公式：$\mathbf {\textbf{增长量=现期量-基期量=基期}\times \mathrm r= \dfrac{\textbf{现期量}}{1+ r}\times r}$  
...
公式：$\mathbf{{r}_{\small{\text{间隔}}}=r_{1}+r_{2}+r_{1}\times r_{2}}$ （和 + 积）  
速算：  
...
特征：间隔一年，求倍数   
两步走： 
① 先求 $\mathbf {r_{\small{间}}}$    
②  $\textbf{间隔倍数}\mathbf{=r_{\small{\textbf {间}}}+1}$   

## 三、间隔基期

特征：间隔一年，求基期  
两步走  
① 先求 $\mathbf {r_{\small{\textbf间}}}$    
② $\textbf {间隔基期}=\dfrac{\textbf{现期量}}{1+\mathbf r_{\small{\textbf{间}}}}$

## 四、间隔增长量

特征：间隔一年，求增长量  
两步走：  
① 先求 $\mathbf {r_{\small {\textbf 间} }}$    
...
特征：间隔一年，求现期量  
两步走：  
① 先求 $\mathbf {r_{\small {\text 间}}}$    
② $\textbf{间隔现期量=基期}\times\quad(1+\mathbf{r}_{\small{\textbf{间}}})$

## 六、间隔多年增长率

方法：多间隔几次。  
小技巧：==当各年 $\mathbf {r>0}$==，间隔多年增长率 $\mathbf{r}_{\small{\text{间}}}>\mathbf{r}_{1}+\mathbf{r}_{2}+\cdots+\mathbf{r}_{n}$  
$\mathbf {16\%}$ 的间隔增长率为 $\mathbf {34.56\%}$    

## 七、间隔的逆向运用  

1.识别：连续的时间，求增长率 
$\mathbf{{r}_ {\small{\text{间}}}=r_{1}+r_{2}+r_{1}\times r_{2}}$   
2.公式：$\mathbf{r}_2=\dfrac{\mathbf{r}_{\small{\textbf{间}}}{-\mathbf{{r}_1}}}{\mathbf 1+\mathbf{r}_ {\mathbf1}}$    
3.考法   
（1）连续两年：整年和整年   
（2）同比套环比   
（3）同比套年底  
...
增长率关系：$\mathbf {r_{\small A} = r_{\small B} + r_{\small C} + r_{\small B} r_{\small C}}$

## 1、总额=单价×数量

判定：$\textbf{总额=单价 }\times \textbf{数量}$   增长 $\mathbf {+ \%}$ （求总额增长率）
公式：$\mathbf {r_{\small A} = r_{\small B} + r_{\small C} + r_{\small B} r_{\small C}}$
 计算技巧： 
① 寻找 $\mathbf {r_{\small B}、r_{\small C}}$  
② 看选项
注：$\mathbf {r_{\small C}=0 \quad 时,\quad r_{\small B} + r_{\small C} + r_{\small B} r_{\small C}=r_{\small B}}$
...
判定：$\textbf{部分=整体} \times \textbf{比重}$ 增长+ % （求部分的增长率）
公式：$\mathbf {r_{\small A} = r_{\small B} + r_{\small C} + r_{\small B} r_{\small C}}$
...
估算结论：$\mathbf {r_{\small {\text{年均}}}\leq\dfrac{r_{\small {总}}}{n\text{年}}}$  
注：$\mathbf r_{\small 总}$ 为时间段的总增长率  
...
<img src="assets/image-20240829102147548.png" style="zoom: 40%;" />
...
$\dfrac {2021}{ \small{比重 10\%}} + \dfrac {新增} {\small{比重20\%}} =  \dfrac {2022} {\small{10\% < 比重 <20\%}}$
...
| 率的关系 |    $\mathbf {r = \dfrac {a-b}{1+b}}$    | $\mathbf {r_{\small A} = r_{\small B} + r_{\small C}}$ |
|          |                                         |                                                        |
